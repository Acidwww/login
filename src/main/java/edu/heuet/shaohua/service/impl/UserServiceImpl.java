...
import edu.heuet.shaohua.dao.UsersizeDOMapper;
...
public class UserServiceImpl implements UserService  {
    ...
    @Autowired
    private UsersizeDOMapper usersizeDOMapper;
     ...
     */
    @Override
    public Boolean getIfUserByTelephone(String telephone) {
    ...
    */
    @Override
    public UserModel getSizeByTelephone(String telephone) throws BusinessException {
        UserDO userDO=userDOMapper.selectByTelephone(telephone);
        /* userDO的判空处理 */
        if (userDO == null){ throw new BusinessException(EmBusinessError.USER_NOT_EXIST);}
        /* 通过UserDO的id查询到关联表user_password中的关联数据 */
        UsersizeDO usersizeDO=usersizeDOMapper.selectByUserId(userDO.getId());

        /* 调用方法，整合UserDO和UserPasswordDO为userModel并返回给Controller */
        return convertFromSize(usersizeDO);

    }

    private UserModel convertFromSize( UsersizeDO usersizeDO) {
        /* UserDO的判空处理 */
        if (usersizeDO == null){ return null; }
        UserModel userModel = new UserModel();
        /* 先把 userDO 中的数据复制给 userModel */
        BeanUtils.copyProperties(usersizeDO, userModel);
    ...
    */
    @Override
    public void updateSize(UserModel userModel) throws BusinessException {
        if (userModel == null){
            throw new BusinessException(EmBusinessError.PARAMETER_VALIDATION_ERROR);
        }
        ...
        int id=userDO.getId();
        userModel.setId(id);
        UsersizeDO usersizeDOCur=usersizeDOMapper.selectByUserId(id);
        if(usersizeDOCur==null){
            UsersizeDO usersizeDO = convertSizeFromModel(userModel);
            usersizeDOMapper.insertSelective(usersizeDO);
        }else {
            UsersizeDO usersizeDO = convertSizeFromModel(userModel);
            usersizeDO.setId(usersizeDOCur.getId());
            usersizeDOMapper.updateByPrimaryKeySelective(usersizeDO);
        }

    }

    @Override
    public void updateUser(UserModel userModel) throws BusinessException {
        if (userModel == null){
            throw new BusinessException(EmBusinessError.PARAMETER_VALIDATION_ERROR);
        }
        UserDO userDO = convertFromModel(userModel);
    ...
    /**
    * @Description: 实现model-->UsersizeDO
    * @Param: [userModel]
    * @return: edu.heuet.shaohua.dataobject.UsersizeDO
    * @Author: 阿C
    * @Date: 2021/8/29
    */
    private UsersizeDO convertSizeFromModel(UserModel userModel) {
        if (userModel == null){
            return  null;
        }
        UsersizeDO usersizeDO = new UsersizeDO();

        usersizeDO.setButto(userModel.getButto());
        usersizeDO.setHeight(userModel.getHeight());
        usersizeDO.setWaist(userModel.getWaist());
        usersizeDO.setChest(userModel.getChest());
        usersizeDO.setCrotch(userModel.getCrotch());
        usersizeDO.setShoulder(userModel.getShoulder());

        usersizeDO.setUserId(userModel.getId());
        return usersizeDO;
    }
