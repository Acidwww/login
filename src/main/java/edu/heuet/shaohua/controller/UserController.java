...
public class UserController  {
    ...
    @ResponseBody
    public CommonReturnType getUserByTel(@RequestParam(name = "telephone") String telephone) throws BusinessException {
    ...
    public CommonReturnType getOtp(@RequestParam(name = "telephone") String telephone) throws BusinessException {
        ...
        boolean hasRegistered = userService.getIfUserByTelephone(telephone);
        if (hasRegistered) {
    ...
    @ResponseBody
    public CommonReturnType updateUser(
            @RequestParam(name = "telephone") String telephone,
            @RequestParam(name = "name") String name,
            @RequestParam(name = "age") String ageStr,
            @RequestParam(name = "gender") String genderStr,
            @RequestParam(name = "style") String styleStr
    ) throws BusinessException {

        UserModel userModelCur=userService.getUserByTelephone(telephone);
        if(userModelCur==null){
            throw new BusinessException(EmBusinessError.USER_NOT_EXIST);
        }
        // 类型转换，适配数据库
        int age = Integer.parseInt(ageStr);
        ...
        Byte gender = Byte.parseByte(genderStr);
        // 验证码通过后，进行注册流程
        userModelCur.setName(name);
        userModelCur.setGender(gender);
        userModelCur.setAge(age);
        userModelCur.setTelephone(telephone);
        userModelCur.setStyle(style);


        userService.updateUser(userModelCur);
        // 注册成功，只返回success即可
    ...
    @RequestMapping(value = "/insertsize", method = {RequestMethod.POST}, consumes = {CONTENT_TYPE_FORMED})
    @ResponseBody
    public CommonReturnType inserSize(
            @RequestParam(name = "telephone") String telephone,
            @RequestParam(name = "height") double height,
            @RequestParam(name = "crotch") double crotch,
            @RequestParam(name = "waist") double waist,
            @RequestParam(name = "shoulder") double shoulder,
            @RequestParam(name = "chest") double chest,
            @RequestParam(name = "butto") double butto
    ) throws BusinessException {
        UserModel userModel = new UserModel();
        userModel.setHeight(height);
        userModel.setCrotch(crotch);
        userModel.setWaist(waist);
        userModel.setShoulder(shoulder);
        userModel.setChest(chest);
        userModel.setButto(butto);
        userModel.setTelephone(telephone);
        userService.updateSize(userModel);
        return CommonReturnType.create(userModel);
    }

    @RequestMapping(value = "/getsize", method = {RequestMethod.POST}, consumes = {CONTENT_TYPE_FORMED})
    @ResponseBody
    public CommonReturnType getSize(@RequestParam(value = "telephone") String telephone)
            throws BusinessException {
        UserModel userModel = userService.getSizeByTelephone(telephone);
